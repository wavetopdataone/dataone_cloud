...
public interface SysRealTimeMonitoringRepository extends JpaRepository<SysRealTimeMonitoring,Long> {
    ...
    List<SysRealTimeMonitoring> findByJobId(Long jobId, Date optTimeOld, Date optTimeNew);
     //取得速率的峰值
    @Query("select max(s.readRate),max(s.writeRate) from SysRealTimeMonitoring s where s.jobId=:jobId and s.optTime>=:optTimeOld and s.optTime<:optTimeNew")
     List<SysRealTimeMonitoring> findByJobIdAndTime(Long jobId,Date optTimeOld,Date optTimeNew);
    @Modifying
